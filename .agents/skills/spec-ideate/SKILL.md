...
2. **Learnings search** — dispatch `.codex/agents/research/learnings-researcher.md` with a brief summary of the ideation focus.

3. **Issue intelligence** (conditional) — if issue-tracker intent was detected in Phase 0.2, dispatch `.codex/agents/research/issue-intelligence-analyst.md` with the focus hint. If a focus hint is present, pass it so the agent can weight its clustering toward that area. Run this in parallel with agents 1 and 2.
