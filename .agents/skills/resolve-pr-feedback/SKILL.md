...
**For review threads** (`review_threads`): Spawn a `.codex/agents/workflow/pr-comment-resolver.md` agent for each.
...
Spawn a single `.codex/agents/workflow/pr-comment-resolver.md` agent for the thread. Then follow the same commit -> push -> reply -> resolve flow as Full Mode steps 5-6.
