...
- **Broken cross-references** -- agent names that are not fully qualified (e.g., `learnings-researcher` instead of `.codex/agents/research/learnings-researcher.md`). Skill-to-skill references using slash syntax inside a SKILL.md where the standards say to use semantic wording. References to tools by platform-specific names without naming the capability class.
